...
\noindent\textbf{Target-class accuracy ($T_{\text{acc}}$).} Target-class accuracy evaluates performance only on test samples belonging to the targeted source class $c_s$:
\begin{equation}
    T_{\text{acc}} \;=\; \frac{|\{(x, y) \in \mathcal{D}_{\text{test}} : y = c_{s},\; \hat{f}(x) = c_{s}\}|}{|\{(x, y) \in \mathcal{D}_{\text{test}} : y = c_{s}\}|}.
\end{equation}
...
\noindent\textbf{Other-label accuracy ($O_{\text{acc}}$).} Other-label accuracy measures performance on all test samples whose labels are not $c_s$:
\begin{equation}
    O_{\text{acc}} \;=\; \frac{|\{(x, y) \in \mathcal{D}_{\text{test}} : y \neq c_{s},\; \hat{f}(x) = y\}|}{|\{(x, y) \in \mathcal{D}_{\text{test}} : y \neq c_{s}\}|}.
\end{equation}
Together, target-class accuracy ($T_{\text{acc}}$) and other-label accuracy ($O_{\text{acc}}$) show whether the attack mainly affects the target class or also damages the rest of the task.

\noindent\textbf{Overall accuracy (ACC).} Overall accuracy (ACC) is the standard test accuracy over all classes:
\begin{equation}
    \mathrm{ACC} \;=\; \frac{|\{(x, y) \in \mathcal{D}_{\text{test}} : \hat{f}(x) = y\}|}{|\mathcal{D}_{\text{test}}|}.
\end{equation}
