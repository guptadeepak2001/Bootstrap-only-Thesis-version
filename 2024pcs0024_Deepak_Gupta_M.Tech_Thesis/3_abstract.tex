...
Functional Inner Product--Federated Learning, the framework developed in this thesis, is built around one idea: the verifier never needs to see a raw gradient. Working entirely under Paillier encryption, it computes one projection score per client: the Functional Inner Product (FIP) of that client's update against a momentum-tracked reference direction. Once the FIP scalar is decrypted, two checks are applied: a direction filter, then a median--MAD magnitude window. The chapters that follow analyse this protocol along three axes: how much a scalar score actually reveals about the gradient, what region of update-space is still open to an attacker, and whether descent is preserved when the loss is $L$-smooth.
