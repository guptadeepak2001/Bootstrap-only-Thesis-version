...
Because FIP-FL ultimately threshold-tests one scalar per client, one-dimensional robust statistics are all that the audit needs. For a score set $\mathcal{S}=\{s_1,\ldots,s_N\}$,
\begin{equation}
    \tilde{\mu}=\mathrm{median}(\mathcal{S}),\qquad
    \mathrm{MAD}=\mathrm{median}\{|s_i-\tilde{\mu}|:s_i\in\mathcal{S}\}.
\end{equation}
Both the median and the MAD are well known to remain stable when up to roughly half the data is contaminated, which is the regime we care about here. The window the audit applies is the adaptive band
\begin{equation}
    \tau_{\min}^{(t)}=\alpha(\rho^{(t)})\tilde{\mu}^{(t)},\qquad
    \tau_{\max}^{(t)}=\tilde{\mu}^{(t)}+\gamma_{\mathrm{mad}}\mathrm{MAD}^{(t)},
    \label{eq:mad-window}
\end{equation}
with $\gamma_{\mathrm{mad}}=5$. A score that drops below the lower edge is essentially a too-weak positive contribution, while one that shoots above the upper edge points to an update that is aligned all right but is throwing too much weight around.

\section{System and Threat Model}
\label{sec:prelim-threat}

The FIP-FL protocol has four roles. The clients submit encrypted gradients. The Aggregation Server takes whatever ciphertexts the audit has accepted and broadcasts the new model. The FIP Verifier is the role that actually computes the scalar FIP scores and runs the two-stage check. The Trusted Key Authority (TKA) generates the Paillier keys and is allowed to decrypt only those scalar scores and aggregates that the protocol explicitly asks it to. The Verifier may be co-located with the server in practice; in either case, no external auditor enters the trust model.
