...
The three theorems collected below match, one to one, the four design targets stated in Section~4.6 (privacy, Byzantine resilience, and convergence; the efficiency target is a complexity statement and is read off directly from the algorithms above).
...
Any malicious update $\mathbf{g}_{\text{adv}}$ accepted by the FIP-FL filter in round $t$ must satisfy both
\begin{align}
    \langle \mathbf{g}_{\text{adv}}, \mathbf{R}^{(t)} \rangle &> 0 & &\text{(direction constraint)}, \\
    \tau_{\min}^{(t)} \le \langle \mathbf{g}_{\text{adv}}, \mathbf{R}^{(t)} \rangle &\le \tau_{\max}^{(t)} & &\text{(magnitude constraint)},
\end{align}
where $\tau_{\min}^{(t)}$ and $\tau_{\max}^{(t)}$ are computed from the positive-score distribution in that round. Together, these two conditions prevent direction-reversing attacks, scaling-to-zero attacks, and destabilizing large-norm attacks, while keeping the audit process independent of any external auditor.
\end{theorem}

\begin{proof}
The audit first applies the direction test and then the $\tau_{\min}^{(t)}$ to $\tau_{\max}^{(t)}$ magnitude window. Any adversarial update that violates either condition is rejected.

These conditions correspond naturally to three common adversarial strategies. A direct poisoning update of the form $\mathbf{g}_{\text{adv}}\approx-\mathbf{g}_{\text{honest}}$ produces a non-positive score and is removed in the first stage. A heavily down-scaled update may remain positive, but its score becomes too small and falls below $\tau_{\min}^{(t)}$. A large destabilizing update may still align with the reference direction, but its score exceeds $\tau_{\max}^{(t)}=\mu^{(t)}+5\cdot\text{MAD}^{(t)}$, and it is therefore filtered out.
\end{proof}
...
In practice, the FIP-FL filter is designed to make this assumption reasonable. Only updates with positive projection onto the reference direction and controlled scalar magnitude are included in $\Delta_{\text{global}}^{(t)}$. Importantly, the theorem does not require the reference direction to equal the true gradient; it states the average descent condition that the accepted aggregate must satisfy.
\end{proof}
