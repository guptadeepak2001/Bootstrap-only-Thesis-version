...
\newcommand{\OverallAccHead}{\textbf{ACC (\%)}}
\newcommand{\TargetAccHead}{\textbf{$T_{\text{acc}}$ (\%)}}
\newcommand{\OtherAccHead}{\textbf{$O_{\text{acc}}$ (\%)}}

This chapter goes through the empirical results of FIP-FL on the grid defined in Chapter~\ref{Setup:chap:chap6}. Each table reports the relevant round-300 numbers, ACC, $T_{\text{acc}}$, $O_{\text{acc}}$, TPR, and FPR. Accuracies are quoted as percentages, and TPR and FPR are quoted as proportions in $[0,1]$.
...
\begin{tabular}{@{}ccccc@{}}
\toprule
\textbf{AR} & \OverallAccHead & \TargetAccHead & \OtherAccHead & \textbf{TPR} \\
\midrule
...
\begin{figure}[H]
\centering
\subfigure[MNIST ACC trajectory]{\includegraphics[width=0.39\textwidth]{fig_noniid_untargeted_mnist_acc.pdf}}
\hfill
\subfigure[KDDCup99 ACC trajectory]{\includegraphics[width=0.39\textwidth]{fig_noniid_untargeted_kdd_acc.pdf}}
\\[-0.6em]
\subfigure[MNIST malicious-client TPR]{\includegraphics[width=0.39\textwidth]{fig_noniid_untargeted_mnist_tpr.pdf}}
\hfill
\subfigure[KDDCup99 malicious-client TPR]{\includegraphics[width=0.39\textwidth]{fig_noniid_untargeted_kdd_tpr.pdf}}
\caption{Non-IID untargeted attack dynamics over 300 communication rounds. Accuracy converges stably for both datasets, while the bootstrap audit reaches perfect malicious-client detection by the final round.}
...
A targeted label-flipping attack is a more delicate kind of poisoning than the untargeted case. It tries to break one specific source class while letting the global accuracy stay believable, so reading ACC alone would actively hide the damage. The number that matters here is $T_{\text{acc}}$.
...
\begin{table}[H]
\centering
\caption{MNIST, non-IID, targeted label-flipping attack $(c_s=0,c_t=7)$; SenSys 2026 headline targeted-accuracy cell, with the auditor-based value from~\cite{yazdinejad2024robust}.}
...
\toprule
\textbf{AR} & \OverallAccHead & \TargetAccHead & \OtherAccHead & \textbf{TPR} & \textbf{FPR} \\
\midrule
...
\begin{figure}[!htbp]
\centering
\subfigure[MNIST ACC trajectory]{\includegraphics[width=0.39\textwidth]{fig_iid_untargeted_mnist_acc.pdf}}
\hfill
\subfigure[KDDCup99 ACC trajectory]{\includegraphics[width=0.39\textwidth]{fig_iid_untargeted_kdd_acc.pdf}}
\\[-0.6em]
\subfigure[MNIST malicious-client TPR]{\includegraphics[width=0.39\textwidth]{fig_iid_untargeted_mnist_tpr.pdf}}
\hfill
\subfigure[KDDCup99 malicious-client TPR]{\includegraphics[width=0.39\textwidth]{fig_iid_untargeted_kdd_tpr.pdf}}
...
\begin{figure}[H]
\centering
\subfigure[MNIST target-class accuracy]{\includegraphics[width=0.45\textwidth]{fig_iid_targeted_mnist_tacc.pdf}}
\hfill
\subfigure[KDDCup99 target-class accuracy]{\includegraphics[width=0.45\textwidth]{fig_iid_targeted_kdd_tacc.pdf}}
