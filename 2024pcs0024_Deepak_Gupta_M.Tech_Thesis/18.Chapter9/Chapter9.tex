...
The argument the thesis has been making is, in one line, that Byzantine-resilient privacy-preserving federated learning does not actually need an external auditor to decrypt cluster-level structure. FIP-FL swaps that trust anchor for a functional inner product taken between each encrypted client update and a momentum-tracked trusted reference direction. The single scalar that the inner product produces is what feeds the two-stage geometric audit --- the direction filter, which catches label-flip style reversals, and the magnitude window, which catches updates that are either too weak or too aggressive.

The proposed approach is implemented through a simple pipeline. Paillier encryption is used at the cryptographic layer, the verifier computes client scores homomorphically, the server aggregates only the accepted encrypted updates, and the reference direction is updated after each round using momentum. The theoretical analysis lines up with this design --- scalar-gradient privacy, a geometric admissibility condition for any malicious update, and a monotone loss-decrease under $L$-smoothness. Empirically, FIP-FL ends up detecting every malicious-client configuration in the completed grid, lifts MNIST non-IID target-class accuracy from roughly $7\%$ in the undefended baseline to $97.69\%$ at a $50\%$ malicious-client rate, and replaces the auditor's $\mathcal{O}(d^3)$ covariance-inversion step with a verification stage that parallelises to a $13\times$ wall-clock speed-up in Phase~II.
